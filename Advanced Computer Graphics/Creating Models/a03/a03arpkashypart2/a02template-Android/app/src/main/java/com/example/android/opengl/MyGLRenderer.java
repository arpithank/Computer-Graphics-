    ...
    public void buildSplineCurve()
    {
        ...
        for(int i=0;i<VerNo-3;i=i+3)
        {
            ...
            for (float t=0.01f;t<=1;t+=0.01) {
                ...
                float x10 = linearInterpolation(x0, x1, t);
                float x11 = linearInterpolation(x1, x2, t);
                float x12 = linearInterpolation(x2, x3, t);
                float y10 = linearInterpolation(y0, y1, t);
                float y11 = linearInterpolation(y1, y2, t);
                float y12 = linearInterpolation(y2, y3, t);

                //Comment: 2nd level of interpolation reduces 3 outputs to 2 outputs
                float x20 = linearInterpolation(x10, x11, t);
                float x21 = linearInterpolation(x11, x12, t);
                float y20 = linearInterpolation(y10, y11, t);
                float y21 = linearInterpolation(y11, y12, t);


                //Comment: last level returns interpolation of last two outputs
                float x = linearInterpolation(x20, x21, t);
                float y = linearInterpolation(y20, y21, t);
                splineData[splineCount]=x;
                splineData[splineCount+1]=y;
                splineCount++;
                */
