...
private int VerNo =0;
private int vflag=0,lflag=0,cflag=0,rflag=0;
        private int v,e;
    private float dx,dy;
    private float centroidx=0, centroidy=0;
    private float sumdx=0,sumdy=0;
    private int pointerdx, pointerdy;
    private float angle=0;
            private int pointercx,pointercy,pointerangle;
    ...
    private void setupGL() {
        ...
        this.pointerdx = GLES20.glGetUniformLocation(this.myGLESProgram, "u_TX");
        this.pointerdy = GLES20.glGetUniformLocation(this.myGLESProgram, "u_TY");
        this.pointerangle = GLES20.glGetUniformLocation(this.myGLESProgram, "u_Angle");
        this.pointercx = GLES20.glGetUniformLocation(this.myGLESProgram, "u_centroidx");
        this.pointercy = GLES20.glGetUniformLocation(this.myGLESProgram, "u_centroidy");
    ...
    private void draw() {
        ...
        if(cflag==1&&rflag==0) {
            GLES20.glUniform1f(this.pointerdx, this.sumdx);
            GLES20.glUniform1f(this.pointerdy, this.sumdy);
        }
        if(rflag==1)
        {
            GLES20.glUniform1f(this.pointercx, this.centroidx);
            GLES20.glUniform1f(this.pointercy, this.centroidy);
            GLES20.glUniform1f(this.pointerangle, this.angle);

        }
        // set vertex coordinates:

        float tempx=gVertexData[0];
        float tempy=gVertexData[1];

        gVertexData[0]=centroidx;
        gVertexData[1]=centroidy;
        this.myVertexBuffer.put(gVertexData);
        this.myVertexBuffer.position(0);
    ...
    */
    ...
    public void proximity(double p, double q) {
        ...
        if (VerNo >= 2) {
            ...
            dist = Math.pow((p - centroidx) * (p - centroidx) + (q - centroidy) * (q - centroidy), 0.5);
    ...
    public void transformations()
    {
        ...
        for(int i=0;i<VerNo;i++) {
            sumx += gVertexData[i * 2];
            sumy += gVertexData[i * 2 + 1];
        }
        centroidx=sumx/VerNo;
        centroidy=sumy/VerNo;
    ...
    public void touchesBegan(float x, float y) {
        ...
        if(VerNo<16 && vflag==0 && lflag==0 && cflag==0)
        {
    ...
    if(cflag==1)
    {
        sumdx+=myTouchXcurrent-myTouchXold;
        sumdy+=myTouchYcurrent-myTouchYold;
    }
        if(rflag==1)
        {
            angle+=(myTouchXcurrent-myTouchXold)*0.01;
        }
