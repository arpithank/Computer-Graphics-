    ...
    public void proximity(double p, double q) {
        ...
        if (VerNo >= 2) {
            for (int j = 0; j < VerNo; j++) {
                ...
                /*double nx = (-(gVertexData[j + 3] - gVertexData[j + 1]));
                double ny = (gVertexData[j + 2] - gVertexData[j]);
                double n = Math.pow((nx * nx) + (ny * ny), 0.5);
                double h = (nx / n) * (p - gVertexData[j]) + (ny / n) * (q - gVertexData[j + 1]);

                double vx = (gVertexData[j + 2] - gVertexData[j]);
                double vy = (gVertexData[j + 3] - gVertexData[j + 1]);
                double v = Math.pow((vx * vx) + (vy * vy), 0.5);
                double l = (vx / v) * (p - gVertexData[j]) + (vy / v) * (q - gVertexData[j + 1]);
                if ((h < 20 || h > -20) && (l > 0 && l < v)) {
                    lflag = 1;
                    e = j;
